...
const getAllCityByStateId = async (stateId) => {
    try {
      ...
      if (error) {
        ...
        return { cities: [] }; // Return an empty array if there was an error
      }

    
  
      return { cities }; // Return the cart_items object as part of the response
    } catch (error) {
      console.error("Error in fetching cart:", error);
      return { cities: [] }; // Return empty array in case of any other errors
    }
  };
   

export default getAllCityByStateId;
